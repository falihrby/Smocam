...
// Helper Functions and Child Components (AreaSelectorAndStream, RightColumn, dll.)
// tidak perlu diubah, jadi saya sembunyikan untuk keringkasan.
// Pastikan Anda menggunakan kode komponen anak yang sudah ada.
// ... (Kode untuk formatTimestamp, fetchWebRTCStream, DashboardCard, RightColumn, AreaSelectorAndStream tetap sama)
...
async function fetchWebRTCStream(videoElement, backendOfferUrl, streamPayload) {
    ...
    return new Promise((resolve, reject) => {
        ...
        (async () => {
            try {
                ...
                const fullPayload = { ...streamPayload, sdp: offer.sdp, type: offer.type };
...
const RightColumn = ({ detectedInfoData }) => (
    <div className="dashboard-column right-column">
        <div className="detected-today-header">
            <h4 className="column-title">Terdeteksi Baru-baru Ini</h4>
        ...
        <div className="detections-list">
            {detectedInfoData.length > 0 ? detectedInfoData.map((info) => (
                ...
                <div className="no-detections-message">Belum ada deteksi baru-baru ini.</div>
            )}
        </div>
    </div>
);
...
const AreaSelectorAndStream = () => {
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(areasQuery, (snap) => {
    ...
    useEffect(() => {
        ...
        const startStream = async (device) => {
            ...
            if (!device.rtspUrl || !device.area || !device.cameraName) {
                setStreamError(`Data CCTV tidak lengkap untuk area "${selectedArea}".`);
                return;
            }
...
const DashboardPage = () => {
    ...
    const cardData = [
        ...
        { icon: '👨‍🎓', number: 15, label: "Mahasiswa" },
    ];
    ...
    return (
        <div className={`dashboard-container ${sidebarOpen ? "" : "sidebar-closed"}`}>
            ...
            <div className="dashboard-main">
                ...
                {/* PERBAIKAN: Menggunakan div wrapper untuk konten agar layout lebih mudah diatur */}
